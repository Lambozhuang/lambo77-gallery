# /bin/bash
find ./local-assets/original -mindepth 2 -maxdepth 2 -type f -name '*.png' -o -name '*.jpg' -exec sh -c '
  rm -rf ./local-assets/photos
  for f do
    source=$f
    output=`echo $source | sed "s/original/photos/g"`
